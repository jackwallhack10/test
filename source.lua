    ...
    -- library functions 
        function library:window(properties) 
            ...
            function cfg.set_menu_visibility(bool, pl)
                ...
                -- Only show playerlist if the toggle is on
                if flags["show_playerlist"] then
                    playerlist.Visible = bool 
                else
        ...
        function library:new_keybind(properties)
            local cfg = {
                text = properties.name or properties.text or "[mb1] aimbot (hold)",
                _type = "keybind",
                key = properties.key or Enum.KeyCode.Unknown,
            ...
            function cfg:set_key(keyCode)
                if keyCode == nil then return self end
                self.key = keyCode
                return self
            end
            
            function cfg:set_mode(mode)
                if mode == nil then return self end
                self.mode = mode
                return self
            end
            
            function cfg:set_active(active)
                if active == nil then return self end
                self.active = active
                return self
            end
        ...
        function library:keybind(properties)
            ...
            flags[cfg.flag] = {
                _type = "keybind",
                key = cfg.key or Enum.KeyCode.Unknown,  -- Use a default key if none provided
                mode = cfg.mode,
                active = cfg.active
            } 

            -- Initialize keybind with default values first
            local key = library:new_keybind({
                name = cfg.keybind_name or ""
            })
            
            -- Ensure the flags table has the keybind registered with proper defaults
            local flagData = {
                _type = "keybind",
                key = cfg.key or Enum.KeyCode.Unknown,
            ...
            flags[cfg.flag] = flagData
            
            -- Set up keybind methods
            function key:set_key(keyCode)
                if keyCode == nil then return end
                flagData.key = keyCode
            end
            
            function key:set_mode(mode)
                if mode == nil then return end
                flagData.mode = mode
            end
            
            function key:set_active(active)
                if active == nil then return end
                flagData.active = active
            end
            
            -- Set initial values if they were provided
            key:set_key(cfg.key)
            key:set_mode(cfg.mode)
